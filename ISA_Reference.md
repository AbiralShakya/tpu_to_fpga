...
```python
program = [
    # Layer 1: Input(4×256) → Hidden(4×256)
    (0x01, 0x00, 0x04, 0x00, 0b00),  # Load input activations
    (0x03, 0x00, 0x04, 0x00, 0b00),  # Load layer 1 weights
    (0x30, 0x0C, 0x00, 0x10, 0b00),  # Wait for data
    (0x10, 0x00, 0x20, 0x04, 0b00),  # MATMUL
    (0x18, 0x20, 0x40, 0x04, 0b00),  # RELU → UB[0x40]
    (0x30, 0x02, 0x00, 0x10, 0b00),  # Wait VPU

    # Layer 2: Hidden(4×256) → Output(4×256)
    (0x03, 0x04, 0x04, 0x00, 0b01),  # Load layer 2 weights (parallel!)
    (0x10, 0x40, 0x60, 0x04, 0b00),  # MATMUL on ReLU outputs
    (0x18, 0x60, 0x80, 0x04, 0b00),  # RELU → final output
    (0x30, 0x02, 0x00, 0x10, 0b00),  # Wait VPU
    (0x02, 0x80, 0x04, 0x00, 0b00),  # Send results to host
    (0x3F, 0x00, 0x00, 0x00, 0b00),  # HALT
]
```
...
program = [
    ...
    (0x01, 0x00, 0x10, 0x00, 0b00),  # Load 4096 bytes
    (0x03, 0x00, 0x08, 0x00, 0b00),  # Load 8 weight tiles
    (0x30, 0x0C, 0x00, 0x20, 0b00),  # Wait with longer timeout
    (0x10, 0x00, 0x20, 0x10, 0b00),  # Larger matrix: 16×256
    (0x18, 0x20, 0x40, 0x10, 0b01),  # Leaky ReLU using cfg[1]
    (0x20, 0x40, 0x50, 0x00, 0b00),  # Max pool using cfg[1] params
    (0x02, 0x50, 0x04, 0x00, 0b00),  # Output results
    (0x3F, 0x00, 0x00, 0x00, 0b00),  # HALT
]
```
